const quotes = [ //array of quotes and authors
  {
...
window.onload = function () { //when the page is loaded
  let random = Math.floor(Math.random() * quotes.length); //get a random number between 0 and the length of the array
  document.getElementById("quote").innerHTML = quotes[random].quote; //get the quote of the random number
  document.getElementById("author").innerHTML = quotes[random].author; //get the author of the random number
};

function genquote() { //when the button is clicked 
  let random = Math.floor(Math.random() * quotes.length); //get a random number between 0 and the length of the array
  document.getElementById("quote").innerHTML = quotes[random].quote; //get the quote of the random number
  document.getElementById("author").innerHTML = quotes[random].author; //get the author of the random number
}